...
import { createDonorPFSchema } from '@/schemas/user.schema'
...
export async function POST(request: NextRequest) {
  ...
  const parsed = createDonorPFSchema.safeParse(body)
  if (!parsed.success) {
  ...
  try {
    const user = await userService.createDonorPF(parsed.data, ip)
    return Response.json(
