...
export default function CadastroPatrocinadorPage() {
  ...
  async function onSubmit(data: FormData) {
    setLoading(true)
    try {
      await signUp({ ...data, role: "PATROCINADOR" })
      toast.success("Bem-vindo(a)! Verifique seu e-mail.")
    } catch {
    ...
    <AuthLayout title="Quero patrocinar" subtitle="Cadastro rápido — 1 etapa">
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
        <div className="space-y-1">
          ...
          <Input id="name" {...register("name")} placeholder="Seu nome" aria-invalid={!!errors.name} />
        ...
        <div className="space-y-1">
          ...
          <Input id="email" type="email" {...register("email")} placeholder="seu@email.com" aria-invalid={!!errors.email} />
          {errors.email && <p className="text-xs text-[var(--color-danger)]">{errors.email.message}</p>}
        </div>
        <div className="space-y-1">
          <Label htmlFor="password">Senha</Label>
          <Input id="password" type="password" {...register("password")} placeholder="Mínimo 8 caracteres" aria-invalid={!!errors.password} />
