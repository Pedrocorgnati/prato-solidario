...
import { createReceptorSchema } from '@/schemas/user.schema'
...
export async function POST(request: NextRequest) {
  ...
  const parsed = createReceptorSchema.safeParse(body)
  if (!parsed.success) {
  ...
  try {
    const user = await userService.createReceptor(parsed.data, ip)
    return Response.json(
